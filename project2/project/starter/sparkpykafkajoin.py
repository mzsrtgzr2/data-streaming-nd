...
df_parsed_json = df.select( \
  col("key").cast("string"),
  from_json(col("value").cast("string"), redis_server_change_schema).alias('value'))
...
df_parse_json_exploded = df_parsed_json.select(col('value.*'))
...
df_parse_json_exploded.createOrReplaceTempView('RedisSortedSet')
...
df_parse_customers.createOrReplaceTempView('CustomerRecordsValid')
